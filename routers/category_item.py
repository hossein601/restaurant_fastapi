...
from schemas.category_item import  CategoryItemResponse, CategoryItemListResponse, \
    CategoryItemUpdate, Category_item_to_category, CategoryItemUpdateOut

category_item_router = APIRouter()

@category_item_router.get("/category_item/{category_id}/", status_code=status.HTTP_200_OK, response_model=CategoryItemListResponse ,
...
def get_items_for_category(category_id: int,db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
    ...
    return CategoryItemListResponse(category_items = categoyItemOut)


@category_item_router.post("/category_item/{category_id}/{item_id}",response_model=Category_item_to_category, status_code=status.HTTP_201_CREATED,
                           dependencies=[Depends(role_checker(["admin"]))])
def assign_item_to_category(category_id: int,item_id: int,db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
...
@category_item_router.put("/category_item/{id_category}/{id_item}", status_code=status.HTTP_201_CREATED,
                          response_model=CategoryItemUpdateOut, dependencies=[Depends(role_checker(["admin"]))])
def update_item_categories(data:CategoryItemUpdate,id_category: int,id_item: int ,db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
    category_item = db.query(CategoryItem).filter(CategoryItem.category_id == id_category, CategoryItem.item_id == id_item).one_or_none()
